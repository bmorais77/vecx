...
fn type_erased_vec_remove<F>(index: F, size: usize) -> Duration
where
    F: Fn(usize) -> usize,
{
...
pub fn bench_remove(c: &mut Criterion) {
    {
        ...
        for size in SIZES.into_iter() {
            bench_custom(
                ...
                &size,
                |&size| type_erased_vec_remove(|_| 0, size),
            );
    ...
    {
        let mut group = c.benchmark_group("remove_back");
        for size in SIZES.into_iter() {
            bench_custom(
                &mut group,
                BenchmarkId::new("TypeErasedVec", size),
                &size,
                |&size| type_erased_vec_remove(|i| i, size),
            );
